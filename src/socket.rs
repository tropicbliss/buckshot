...
pub async fn snipe_gc(
    ...
    for _ in 0..6 {
        ...
        let handle =
            task::spawn(
                async move { snipe_task_gc(snipe_time, username_to_snipe, access_token).await },
            );
        handle_vec.push(handle);
    }
    for handle in handle_vec {
...
pub async fn auto_offset_calculation_regular(username_to_snipe: &str) -> i32 {
    let data = format!("PUT /minecraft/profile/name/{} HTTP/1.1\r\nHost: api.minecraftservices.com\r\nConnection: keep-alive\r\nContent-Length: 0\r\nAccept: */*\r\nAuthorization: Bearer token\r\nUser-Agent: {}\r\n", username_to_snipe, constants::USER_AGENT);
    let data = data.as_bytes();
    let data2 = format!("\r\n");
    let data2 = data2.as_bytes();
    let stream = TcpStream::connect("api.minecraftservices.com:443")
